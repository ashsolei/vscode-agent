...
| **Docker** | Multi-stage Dockerfile för containerisering |
...
Bygg och kör extensionen i en Docker-container:
...
# Tagga och pusha till lokal registry
docker tag vscode-agent localhost:5000/vscode-agent:latest
docker push localhost:5000/vscode-agent:latest
```

Dockerfile använder multi-stage build:
1. **Builder** — Installerar beroenden och kompilerar TypeScript
2. **Packager** — Kopierar kompilerad kod och prunar devDependencies
3. **Runtime** — Minimal image med non-root user
...
| Middleware | 6 | pipeline, timing, usage, rate-limit |
| WorkflowEngine | 4 | sequential, parallel, conditional |
| GuardRails | 4 | checkpoint, rollback, dry-run |
